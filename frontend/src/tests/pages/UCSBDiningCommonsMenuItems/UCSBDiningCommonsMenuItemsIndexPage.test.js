...
describe("MenuItemIndexPage tests", () => {
  ...
  test("Renders with Create Button for admin user", async () => {
    setupAdminUser();
    axiosMock.onGet("/api/menuitems/all").reply(200, []);
    ...
    expect(button).toHaveAttribute("href", "/menuItem/create");
  ...
  test("renders empty table when backend unavailable, user only", async () => {
    ...
    axiosMock.onGet("/api/menuitems/all").timeout();
    ...
    expect(errorMessage).toMatch(
      "Error communicating with backend via GET on /api/menuitems/all",
    );
    restoreConsole();
  });
  ...
  test("what happens when you click delete, admin", async () => {
    ...
    expect(axiosMock.history.delete[0].url).toBe("/api/menuitems");
    expect(axiosMock.history.delete[0].url).toBe("/api/menuitems");
