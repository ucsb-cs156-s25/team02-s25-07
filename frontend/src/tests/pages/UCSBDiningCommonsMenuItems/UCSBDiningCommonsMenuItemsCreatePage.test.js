...
describe("UCSBDiningCommonsMenuItemCreatePage tests", () => {
  ...
  test("on submit, makes request to backend, and redirects to /menuItems", async () => {
    const queryClient = new QueryClient();
    const item = {
      id: 2,
    ...
    axiosMock.onPost("/api/menuitems/post").reply(202, item);
    ...
    fireEvent.change(nameInput, { target: { value: "California Roll" } });
    fireEvent.change(stationInput, { target: { value: "International" } });
    ...
    expect(mockToast).toHaveBeenCalledWith(
      "New item Created - id: 2 name: California Roll",
    );
